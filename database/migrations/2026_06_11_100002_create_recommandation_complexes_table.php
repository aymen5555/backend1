<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::create('recommandation_complexes', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained('users')->cascadeOnDelete();
            $table->foreignId('complexe_id')->constrained('complexes')->cascadeOnDelete();
            $table->unsignedTinyInteger('score');         // 0-100
            $table->unsignedTinyInteger('rang');          // 1, 2, or 3
            $table->text('explication');                  // human-readable explanation
            $table->timestamps();

            $table->index(['user_id', 'rang']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('recommandation_complexes');
    }
};
